...
const bbox = "25.440,81.820,25.460,81.845"; // Expanded to ensure no edge cutoff before poly logic
const query = `
[out:json][timeout:25];
(
...
// Expanded bounding polygon matching user's blue stroke
const polygon = [
  [81.824, 25.443], // SW: Nawab Yusuf & Sarojini Naidu
  [81.824, 25.457], // NW: Muir & Sarojini Naidu
  [81.841, 25.457], // NE: Thornhill & Kamla Nehru
  [81.841, 25.443], // SE: Kamla Nehru & Nawab Yusuf
];
...
https.get(url, (res) => {
  ...
  res.on('end', () => {
    try {
        ...
        // Remove clusters that are too close to each other (sub 120m apart)
        let finalSet = [];
        for (let c of clusters) {
            let tooClose = finalSet.some(f => distHaversine(f.lat, f.lon, c.lat, c.lon) < 120);
            if (!tooClose) finalSet.push(c);
        }
        
        console.log("Found " + finalSet.length + " major intersections in widened polygon");
        ...
        console.log("Written updated intersections to data/civilLinesSignals.ts");
